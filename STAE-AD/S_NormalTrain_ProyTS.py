    ...
    for model_name in model_names:
        ...
        for x_train_normal,x_val_nomal in datatotrain (model_name,train_data_model01,train_data_model02,train_data_model04):
            ...
            if (model_name == 'ModelWithNoise1') or (model_name == 'SpatioTemporalwithAttSimpleModel2withNoise') or (model_name == 'CNN_Noise1'):
                ...
                x_val_nomal_n = x_val_nomal       
                # compile model
            model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0, clipnorm=1, amsgrad=True), loss=args.loss, metrics=args.metrics)
            model.summary()
            ...
            mc = tf.keras.callbacks.ModelCheckpoint(h5model[countmol], monitor='val_root_mean_squared_error', mode='min', verbose=1,save_best_only=True) 
            time_callback = TimeHistory()
            hytories = []
            if stateful:
